...
time_custom::unix_timestamp time_custom::readabel_convert_to_unix_timestamp(const time_custom::readable &readable)
{
    // 946656000 is the Unix timestamp of 2000-01-01 00:00:00
    time_custom::unix_timestamp unix_timestamp{unix_timestamp_of_2000_01_01_00_00, readable.millisecond};
    uint16_t day = 0;
    // add the seconds, minutes and hours to the UnixTimestamp
    unix_timestamp.second += readable.hour * 60 * 60 + readable.minutes * 60 + readable.seconds;
    ...
    // calculate the number of days of the previous years, notice 2000 is a leap year
    day += readable.year * 365 + readable.year / 4 - readable.year / 100 + readable.year / 400 + 1 + readable.month_day - 1
            - (((readable.month <= 2) && (readable.year % 400 == 0 || (readable.year % 4 == 0 && readable.year % 100 != 0))) ? 1 : 0);
    unix_timestamp.second += day * 60 * 60 * 24;
    return unix_timestamp;
}

time_custom::readable time_custom::unix_timestamp_convert_to_readable(const time_custom::unix_timestamp &unix_timestamp)
{
    // 946656000 is the Unix timestamp of 2000-01-01 00:00:00
    uint32_t unix_timestamp_second = unix_timestamp.second - 946656000;
    time_custom::readable readable{unix_timestamp.millisecond, 0, 0, 0, 0, 0, 1, 0};
    uint32_t day = 0;
    ...
    while(true)
    {
        days_this_year = 365 + ((readable.year % 400 == 0 || (readable.year % 4 == 0 && readable.year % 100 != 0)) ? 1 : 0);
    ...
    while(true)
    {
        days_this_month = days_of_each_month[readable.month - 1] + ((readable.month == 2 && ((readable.year % 4 == 0 && readable.year % 100 != 0) || readable.year % 400 == 0)) ? 1 : 0);
...
bool time_custom::is_readable_valid(const time_custom::readable &readable)
{
    ...
    if(readable.month_day < 1 || readable.month_day > days_of_each_month[readable.month - 1] + ((readable.month == 2 && ((readable.year % 4 == 0 && readable.year % 100 != 0) || readable.year % 400 == 0)) ? 1 : 0))
    {
        return false;
    }
...
void time_custom::make_readable_valid(time_custom::readable &readable)
{
    ...
    else if(readable.month_day > days_of_each_month[readable.month - 1] + ((readable.month == 2 && ((readable.year % 4 == 0 && readable.year % 100 != 0) || readable.year % 400 == 0)) ? 1 : 0))
    {
        readable.month_day = days_of_each_month[readable.month - 1] + ((readable.month == 2 && ((readable.year % 4 == 0 && readable.year % 100 != 0) || readable.year % 400 == 0)) ? 1 : 0);
    }
...
uint8_t time_custom::calculate_week_day(const time_custom::unix_timestamp &unix_timestamp)
{
    // 2000-01-01 is saturday
...
{
    static_cast<time_custom::time *>(time)->unix_timestamp_add_millisecond(millisecond);
}
...
{
    static_cast<time_custom::time *>(time)->set_readable(readable);
}

void time_set_unix_timestamp(void *time, time_custom_unix_timestamp unix_timestamp)
{
   static_cast<time_custom::time *>(time)->set_unix_timestamp(unix_timestamp);
}
